...
TEST_CASE("Test density is nonzero for particles"){
	...
	double density = get_density_with_uniform_h<CubicSplineKernel3>(center_idx, std::span<const size_t>(neighbor_indices), dsd, kernel);
	REQUIRE(density > 0.0);
}
...
TEST_CASE("profile std::accumulate vs for loop "){
	...
	BENCHMARK("std::accumulate") {
		return get_density_with_uniform_h<CubicSplineKernel3>(center_idx, std::span<const size_t>(neighbor_indices), dsd, kernel);
	};
	BENCHMARK("for loop") {
		return get_density_with_uniform_h_silly_loop<CubicSplineKernel3>(center_idx, std::span<const size_t>(neighbor_indices), dsd, kernel);
	};
}
